...
On the first start against an empty database, the application creates the tables and inserts a **demonstration data set** (see `core/seed.py`): 22 users (administrators, a program manager, a campus manager, and students), four surveys with their submissions and answers, and one prompt. It seeds no `facilitator` and no summaries. The programs from `import/Program_list.csv`, the default LLM provider and the known model prices are synchronised on every start.
