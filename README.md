...
2. **Run it**, either way:
   ...
   - **Without Docker**: [uv](https://docs.astral.sh/uv/) and Python **3.12** (declared in `.python-version`). `uv sync` creates `.venv`, installs the dependencies pinned in `uv.lock` and the `oceens` package itself; then start `uvicorn oceens.main:app` from that environment (smoke test, step 1). The smoke test calls the environment's executables by their path rather than activating it, because PowerShell blocks `Activate.ps1` by default. `uv run oceens` also works, from any folder of the repository.
...
- or run it yourself with `uv run oceens-summaries-daemon`, its installed entry point.
...
- `./src/oceens/import` on `/app/src/oceens/import`, the seed's CSV files (the demonstration answers are kept out of the image by `.dockerignore`).

The image installs the dependencies from `uv.lock` (`uv sync --locked`), copies `src/` in at build time, sets `LOCAL_DATABASE_DIR=/app/database`, and runs `uvicorn oceens.main:app` **without** `--reload`: after changing the code, rebuild (`docker compose up --build`).
...
`launch.sh` is the production launcher without Docker, on the school's server: it runs `uv sync --locked`, then the two installed entry points, `oceens` (the application) and `oceens-summaries-daemon`, in two separate `screen` sessions. It is written for that machine only: it `cd`s into a hard-coded path (`/home/mde-admin/OceENS`).

---

## Configuration

Every setting is an environment variable, read from `.env` at startup (`load_dotenv()` in `main.py`, `core/auth.py` and the daemon; `.env` is looked for from the package's folder upwards, so it is found at the repository root whatever the working directory). **[`.env.example`](.env.example) is the reference**: copy it, and read its comments. Variables the code reads:
...
2. **Restart the summaries daemon** (`uv run oceens-summaries-daemon`): it reads `.env` only at startup.
...
├── .python-version               # Interpreter: 3.12
├── launch.sh                     # Production launcher on the school's server, without Docker
├── Dockerfile                    # Application image, installed from uv.lock
...
│
├── src/oceens/                   # The one root package: every import is `oceens.…`
...
│   ├── summaries_generator_daemon.py  # LLM summary processing (separate process); `oceens-summaries-daemon` entry point
│   │
...
│   │   └── llm/                  #     LLM administration: _access.py, providers.py, prices.py, costs.py
│   │
...
│
└── .venv/                        # Virtual environment created by `uv sync` (not committed)
```

The package reads its templates, static files and seed data relative to itself, not to the working directory, so the application starts from any folder. Paths such as `core/seed.py` elsewhere in this README are relative to `src/oceens/`.
...
- [ ] Summaries daemon (`oceens-summaries-daemon`) running if LLM summaries are used
